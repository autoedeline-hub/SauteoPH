-- 2026-09-20: invited dine-in guests may change party size freely at booking,
-- bounded only by what the chosen slot can seat.
--
-- 20260718120000_dine_in_reduce_pax_capped let a guest REDUCE below the invite's
-- group_size but clamped any increase (LEAST(payload, invite.group_size)),
-- silently: the form showed 3 guests and booked 2. Eds' decision 2026-09-20:
-- "allow if there is an available slot; if none, tell the guest to choose a
-- different time". The form already greys out times that cannot seat the party
-- and reserve_seats() below still rejects a slot without ceil(party / 2) free
-- tables, so capacity stays enforced server-side.
--
-- Change vs 20260718120000 (ONE behavioral line):
--   - Invited dine-in party size = the payload when present, else the invite's
--     size. No clamp in either direction beyond the 1-50 validation.
...
BEGIN
  ...
  IF NOT v_is_pickup_order THEN
    IF v_invite_token IS NOT NULL AND v_invite.group_size IS NOT NULL THEN
      v_group_size := coalesce(v_group_size, v_invite.group_size);
    END IF;
