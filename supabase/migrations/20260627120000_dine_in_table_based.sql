-- Redesign dine-in booking as table-based reservation.
-- 7 tables, 2 chairs each. Every dine-in booking = 1 table.
--
-- Changes:
--   1. Recalculate time_slots for dine-in channel:
--        capacity    = 7 (tables, not seats)
--        seats_taken = count of non-cancelled bookings (tables taken)
--   2. create_booking RPC:
--        - For dine_in: always passes 1 to reserve_seats (1 table unit)
--        - For dine_in: hardcodes group_size = 2 (2 chairs per table)
--        - Pickup flow unchanged

-- ---------------------------------------------------------------------------
-- 1. Recalculate existing dine-in slots to table-unit counts
-- ---------------------------------------------------------------------------
...
WHERE ts.channel = 'dine_in';

-- ---------------------------------------------------------------------------
-- 2. Recreate create_booking — table-aware dine-in branch
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Dine-in is table-based: always 1 table = 2 chairs, regardless of payload.
  ...
  END IF;
  -- group_size validated for pickup only (dine-in is always 2)
  ...
  -- Dine-in: 1 table unit per booking (capacity = 7 tables)
  -- Pickup:  group_size units (capacity = number of meals)
  v_seats_to_reserve := CASE WHEN v_is_pickup_order THEN v_group_size ELSE 1 END;
