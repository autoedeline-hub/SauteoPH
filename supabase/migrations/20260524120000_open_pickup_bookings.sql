-- Open pickup bookings to the public. Dine-in stays invite-only.
--
-- Sautéo's seating capacity is hand-managed via Messenger waitlist, so
-- dine-in invites stay required to prevent oversold tables. Pickup is
-- same-day with no seating constraint — anyone hitting /pick-up should
-- be able to order without a Messenger handoff.
--
-- Change: create_booking() now only requires an invite_token when the
-- pickup_mode is 'dine_in'. Pickup modes (personal_pickup, lalamove,
-- grab) accept bookings from anon callers without a token. If a token
-- IS provided on a pickup order (legacy /pick-up/<token> link), it's
-- still validated + consumed atomically as before.
