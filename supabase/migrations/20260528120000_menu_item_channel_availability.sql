-- Let admins decide whether each menu item appears in the dine-in menu,
-- pickup menu, or both.
...
ALTER TABLE public.menu_items
  DROP CONSTRAINT IF EXISTS menu_items_at_least_one_channel_check;
ALTER TABLE public.menu_items
  ADD CONSTRAINT menu_items_at_least_one_channel_check
    CHECK (available_dine_in OR available_pickup);

CREATE INDEX IF NOT EXISTS menu_items_active_dine_in_idx
  ON public.menu_items (sort_order)
  WHERE active = true AND available_dine_in = true;

CREATE INDEX IF NOT EXISTS menu_items_active_pickup_idx
  ON public.menu_items (sort_order)
  WHERE active = true AND available_pickup = true;

-- Recreate create_booking() so server-side item validation matches the
-- channel-specific customer menus. This prevents stale carts or crafted
-- requests from booking pickup-only items for dine-in, and vice versa.
